...
import { PageShell, SectionHeading, SubHero } from "../site";
export const metadata: Metadata = { title:"联系我们", description:"通过公司邮箱、联系电话或官网咨询表单联系GLC盛途教育。" };
...
const advisors=[
  {name:"吴老师",phone:"139 8936 2793",phoneHref:"+8613989362793",wechat:"GLC210411"},
  {name:"常老师",phone:"130 8208 9030",phoneHref:"+8613082089030",wechat:"13082089030（微信同号）"},
  {name:"张老师",phone:"132 2860 6858",phoneHref:"+8613228606858",wechat:"zz13228606858"},
];
export default function Contact(){return <PageShell><SubHero kicker="KONTAKT" title="联系我们" intro="先从一次认真沟通开始。告诉我们你的现状、目标和时间线。" index="06"/>
  <section className="section container contact-layout"><div className="contact-intro"><p className="eyebrow">KONTAKTWEGE</p><h2>保持联系</h2><p>GLC盛途教育由重庆盛途教育咨询有限公司运营，业务范围包括德国语言班、公立及私立高中、大学申请，以及未成年学生在德支持服务。</p><div className="channel-list">{channels.map(([name,value])=><div key={name}><span>{name}</span><b>{value}</b></div>)}</div><div className="contact-actions"><a className="button primary" href="tel:+8613527596068">拨打电话</a><a className="button secondary-button" href="mailto:shengtujy@163.com">发送邮件</a></div></div><LeadForm fields={fields} submitLabel="生成咨询邮件" successTitle="咨询邮件已生成" successText="您的设备应已打开邮件编辑窗口。"/></section>
  <section className="section soft-section">
    <div className="container">
      <SectionHeading kicker="DIREKTER KONTAKT" title="添加老师微信，直接沟通。" text="以下联系方式来自GLC德国高中项目资料。添加微信时可备注“德国高中”，并简要说明学生目前年级与计划入学时间。" />
      <div className="advisor-contact-grid">
        <div className="advisor-list">
          {advisors.map((advisor,index)=><article key={advisor.name}>
            <span>0{index+1}</span>
            <div><h3>{advisor.name}</h3><a href={`tel:${advisor.phoneHref}`}>{advisor.phone}</a></div>
            <p><small>微信</small><b>{advisor.wechat}</b></p>
          </article>)}
        </div>
