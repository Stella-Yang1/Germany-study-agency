...
export const metadata: Metadata = { title:"联系我们", description:"通过微信、邮箱、电话、WhatsApp 或联系表单联系德境教育。" };
...
export default function Contact(){return <PageShell><SubHero kicker="KONTAKT" title="联系我们" intro="先从一次认真沟通开始。告诉我们你的现状、目标和时间线。" index="06"/>
  <section className="section container contact-layout"><div className="contact-intro"><p className="eyebrow">KONTAKTWEGE</p><h2>保持联系</h2><p>德境教育由重庆盛途教育咨询有限公司运营，业务范围包括德国语言班、公立及私立高中、大学申请，以及未成年学生在德支持服务。</p><div className="channel-list">{channels.map(([name,value])=><div key={name}><span>{name}</span><b>{value}</b></div>)}</div><p className="contact-note">微信、小红书、Instagram、WhatsApp 等官方账号尚未提供，因此暂不展示，避免使用无法核实的占位账号。</p></div><LeadForm fields={fields}/></section>
